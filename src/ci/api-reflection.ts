...
export class APIReflectionTester {
  ...
  compareAPISurfaces(expected: APISurface, actual: APISurface): APIComparison {
    ...
    for (const key of expectedKeys) {
      if (actualKeys.has(key)) {
        ...
        if (expectedProp.isConfigurable !== actualProp.isConfigurable ||
  ...
  testEdgeCases(papa: any): {
    ...
    const internalClasses = ['Parser', 'ParserHandle', 'NetworkStreamer', 'FileStreamer', 'StringStreamer', 'ReadableStreamStreamer'];
   ...
   */
  runCompleteAPITest(papa: any, version: string): {
    passed: boolean;
    ...
    const singletonTest = { passed: true, details: 'Singleton test skipped (requires require function)' };

    // Test edge cases
    const edgeCaseTests = this.testEdgeCases(papa);
    console.log(`Edge Case Tests: ${edgeCaseTests.passed ? 'PASSED' : 'FAILED'}`);

    const passed = (!comparison || comparison.passed) && 
