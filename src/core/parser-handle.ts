import { copy, isFunction, escapeRegExp } from '../utils/index.js';
import { Parser } from './parser.js';
import { CONSTANTS } from '../constants/index.js';
import type { PapaParseConfig, PapaParseResult, PapaParseParser, PapaParseError } from '../types/index.js';
...
export class ParserHandle implements PapaParseParser {
	...
	private static readonly MAX_FLOAT = Math.pow(2, 53);
	private static readonly MIN_FLOAT = -ParserHandle.MAX_FLOAT;
	private static readonly FLOAT = /^\s*-?(\d+\.?|\.\d+|\d+\.\d+)([eE][-+]?\d+)?\s*$/;
	private static readonly ISO_DATE = /^((\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d\.\d+([+-][0-2]\d:[0-5]\d|Z))|(\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d([+-][0-2]\d:[0-5]\d|Z))|(\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d([+-][0-2]\d:[0-5]\d|Z)))$/;
	...
	private applyDynamicTypingToField(value: any, fieldIndex: number): any {
		...
		if (typeof this.config.dynamicTyping === 'object') {
			if (typeof fieldIndex === 'number' && this.config.dynamicTyping[fieldIndex] === false) {
				return value;
			}
			if (typeof fieldIndex === 'string' && this.config.dynamicTyping[fieldIndex] === false) {
				return value;
			}
	 ...
	 */
	private guessDelimiter(input: string): { successful: boolean; bestDelimiter: string } {
		// Basic implementation - will be enhanced in Phase 3
		const delimiters = this.config.delimitersToGuess || [',', '\t', '|', ';', CONSTANTS.RECORD_SEP, CONSTANTS.UNIT_SEP];
