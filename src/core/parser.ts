import { stripBom, isFunction } from '../utils/index.js';
import { Lexer, Token, TokenType, createLexerConfig } from './lexer.js';
import type { PapaParseConfig, PapaParseResult, PapaParseError, PapaParseParser } from '../types/index.js';
...
export class Parser implements PapaParseParser {
	...
	private processField(value: string): void {
		...
		if (isFunction(this.config.transform)) {
			processedValue = this.config.transform(processedValue, this.state.fieldCount);
		}

		// Apply dynamic typing
		if (this.config.dynamicTyping) {
			processedValue = this.applyDynamicTyping(processedValue, this.state.fieldCount);
		}
